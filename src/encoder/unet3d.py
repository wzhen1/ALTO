...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
        ...
        print('apply_pooling', self.apply_pooling)
        if apply_pooling:
            if pool_type == 'max':
    ...
    def generate_grid_features(self, p, c, channel, reso_grid):
        p_nor = normalize_3d_coordinate(p.clone(), padding=self.padding)
        index = coordinate2index(p_nor, reso_grid, coord_type='3d')
        # scatter grid features from points
        fea_grid = c.new_zeros(p.size(0), channel, reso_grid**3)
        c = c.permute(0, 2, 1)
        fea_grid = scatter_mean(c, index, out=fea_grid) # B x C x reso^3
        fea_grid = fea_grid.reshape(p.size(0), channel, reso_grid, reso_grid, reso_grid) # sparce matrix (B x 512 x reso x reso)
    ...
    def forward(self, x, p, x_after_conv=None, c_last=None):
        # start with (1, 32, 64, 64, 64)
        # print('x================', x['grid'].shape)

        if self.pooling is not None:
            x['grid'] = self.pooling(x['grid'])
            # print('x_after_pooling', x['grid'].shape)
        ...
        if x_after_conv == None:
          ...
          if self.pooling is not None:
            x['grid'] = x['grid'] + self.conv1x1(self.pooling(x_after_conv['grid']))
          else:
            x['grid'] = x['grid'] + self.conv1x1(x_after_conv['grid'])

        x_after_conv = {}
        x_after_conv['grid'] = x['grid']

        c = self.sample_grid_feature(p, x['grid'])
        c = c.transpose(1, 2)
        
        # print('c_before', c.shape)
        c = self.fc_comm(c)
        # print('c_after', c.shape)
        ...
        x['grid'] = self.generate_grid_features(p, c, channel, reso_grid)
        
        # print('xxxxxx', x['grid'].shape)
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_feature_conv1x1, in_channels, out_channels, kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
        ...
        print(self.if_upsampling)
        print(self.unet_levels)
        ...
        if self.if_upsampling:
          if basic_module == DoubleConv:
              ...
              # self.conv1x1 = nn.Conv3d(in_feature_conv1x1, out_channels, 1)

              self.up = Upsampling(transposed_conv=False, in_channels=in_channels, out_channels=out_channels,
                                          kernel_size=kernel_size, scale_factor=scale_factor, mode=mode)
              self.conv1x1 = nn.Conv3d(in_feature_conv1x1, out_channels, 1)
          ...
          # self.upsampling = nn.Conv3d(in_feature_conv1x1, in_feature_conv1x1, 1)
          self.conv1x1 = nn.Conv3d(in_feature_conv1x1, out_channels, 1)
          self.joining = partial(self._joining, concat=True)
        
        print('infeatures', in_channels)
        print('outfeatures', out_channels)

        

        self.fc_c = nn.Linear(in_feature_conv1x1, out_channels)

        # print('inininin', in_channels)
        # print('outouout', out_channels)
    ...
    def forward(self, encoder_features, x, p, x_after_conv, c_last, i):
        # print('x_before_upsampling', x['grid'].shape)
        # print('encoder_features', encoder_features.shape)
        if self.if_upsampling:
          x['grid'] = self.upsampling(encoder_features=encoder_features, x=x['grid'])
        # else:
        #  x['grid'] = self.upsampling(x['grid'])

        # print('x_after_upsampling', x['grid'].shape)
        x['grid'] = self.joining(encoder_features, x['grid'])
        # print('x_concat', x['grid'].shape)
        x['grid'] = self.basic_module(x['grid'])
        # print('x_decoder', x['grid'].shape)
        ...
        if x_after_conv == None:
            ...
            if self.if_upsampling:
              x['grid'] = x['grid'] + self.conv1x1(self.up(x['grid'], x_after_conv['grid']))
            else:
              x['grid'] = x['grid'] + self.conv1x1(x_after_conv['grid'])
        ...
        if i == self.unet_levels - 2:
            # print('x_output_shape======================', x['grid'].shape)
            return x, x_after_conv, c_last 
        # print('=========================i', i)
        # print('x[grid]_______', x['grid'].shape)
        ...
        x['grid'] = self.generate_grid_features(p, c, channel, reso_grid)
        # print('x[grid]==========after', x['grid'].shape)
...
class Upsampling(nn.Module):
    ...
    def __init__(self, transposed_conv, in_channels=None, out_channels=None, kernel_size=3,
        ...
        if transposed_conv:
            ...
            # (D_outÃ¢â‚¬â€¦=Ã¢â‚¬â€¦(D_inÃ¢â‚¬â€¦Ã¢Ë†â€™Ã¢â‚¬â€¦1)Ã¢â‚¬â€¦Ãƒâ€”Ã¢â‚¬â€¦Ã¢â‚¬â€¦stride[0]Ã¢â‚¬â€¦Ã¢Ë†â€™Ã¢â‚¬â€¦2Ã¢â‚¬â€¦Ãƒâ€”Ã¢â‚¬â€¦Ã¢â‚¬â€¦padding[0]Ã¢â‚¬â€¦+Ã¢â‚¬â€¦Ã¢â‚¬â€¦kernel_size[0]Ã¢â‚¬â€¦+Ã¢â‚¬â€¦Ã¢â‚¬â€¦output_padding[0])
...
class Abstract3DUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        # print('reversed_f_maps', reversed_f_maps)
        ...
        for i in range(len(reversed_f_maps) - 1):
            ...
            # print('in_feature_num', reversed_f_maps[i])
            # print('in_channels', in_feature_num)
            # in_feature_num = reversed_f_maps[i]
    ...
    def forward(self, x, p, c):
        ...
        for encoder in self.encoders:
            x, x_after_conv, c = encoder(x, p, x_after_conv, c)
            # reverse the encoder outputs to be aligned with the decoder
            encoders_features.insert(0, x['grid'])
        ...
        for i, (decoder, encoder_features) in enumerate(zip(self.decoders, encoders_features)):
            ...
            x, x_after_conv, c = decoder(encoder_features, x, p, x_after_conv, c, i)

        x['grid'] = self.final_conv(x['grid'])

        # apply final_activation (i.e. Sigmoid or Softmax) only during prediction. During training the network outputs
        # logits and it's up to the user to normalize it before visualising with tensorboard or computing validation metric
        if self.testing and self.final_activation is not None:
            x['grid'] = self.final_activation(x['grid'])
