...
device = torch.device("cuda:0" if is_cuda else "cpu") # zw
# Set t0
t0 = time.time()
...
model.module.encoder.unet = nn.DataParallel(model.module.encoder.unet, device_ids=[0, 1]) # tri-plane 2dcnn
#model.module.encoder.unet3d = nn.DataParallel(model.module.encoder.unet3d, device_ids=[0, 1]) # for volume 3dcnn
...
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=2500, eta_min=learning_rate * 0.0001) # 1875
